...
* daemonize设置为yes后，Redis会把PID写到pidfile文件中
...
* noeviction：不进行移除。针对写操作，只是返回错误信息(永不过期)
* 
